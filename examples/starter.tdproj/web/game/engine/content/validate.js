...
import { LINE_OF_SIGHT_LIMITS } from "./elevation-mechanics.js";
...
export function validateGameContentRegistry(content) {
    ...
    const validateElevationMechanics = () => {
        ...
        if (module.schemaVersion !== 1 && module.schemaVersion !== 2) {
            err("mechanics", "elevation", "modules.elevation.schemaVersion", "Elevation future or unsupported schemaVersion; only versions 1 and 2 are supported.");
        }
        if (typeof module.enabled !== "boolean") {
        ...
        for (const [missionId, profileId] of missionSelections) {
            if (!Object.prototype.hasOwnProperty.call(profiles, profileId)) {
                const report = module.enabled === true && (module.schemaVersion === 1 || module.schemaVersion === 2) ? err : warn;
        ...
        for (const profileId of Object.keys(profiles).sort()) {
            ...
            if (module.schemaVersion !== 2)
                continue;
            unknownFields(profile, ["lineOfSight"], profileId, root);
            ...
            if (!Array.isArray(rawTags) || Object.getPrototypeOf(rawTags) !== Array.prototype) {
            ...
            if (rawTags.length > LINE_OF_SIGHT_LIMITS.terrainBlockerTags) {
        ...
        const activeLineOfSightMissionIds = module.enabled === true && module.schemaVersion === 2

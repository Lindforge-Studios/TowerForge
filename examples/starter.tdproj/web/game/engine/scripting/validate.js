...
function validateScript(key, script, refs, report) {
    ...
    if (script.schemaVersion !== 1 && script.schemaVersion !== 2 && script.schemaVersion !== 3 && script.schemaVersion !== 4 && script.schemaVersion !== 5) {
        report(scriptId, "schemaVersion", "TowerScript schemaVersion must be 1, 2, 3, 4, or 5.");
    }
    ...
    for (const [event, handlers] of Object.entries(script.handlers)) {
        ...
        if (event === "enemyMarkChanged" && script.schemaVersion !== 4 && script.schemaVersion !== 5) {
            report(scriptId, `handlers.${event}`, `TowerScript event "${event}" requires schemaVersion 4.`);
        }
        if ((event === "enemyExposureChanged" || event === "enemyReactionTriggered") && script.schemaVersion !== 5) {
        ...
        handlers.forEach((handler, index) => {
            ...
            if (!Array.isArray(handler.actions) || handler.actions.length === 0)
                ...
                const actionSchemaVersion = event === "enemyReactionTriggered" && script.schemaVersion !== 5
                    ? 5
...
function validateAction(scriptId, path, action, schemaVersion, refs, report) {
    ...
    if ((action.action === "applyEnemyMark" || action.action === "clearEnemyMark") && schemaVersion !== 4 && schemaVersion !== 5) {
        report(scriptId, `${path}.action`, `TowerScript action "${action.action}" requires schemaVersion 4.`);
    }
    if ((action.action === "applyEnemyExposure" || action.action === "clearEnemyExposure") && schemaVersion !== 5) {

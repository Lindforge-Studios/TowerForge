...
import { LINE_OF_SIGHT_LIMITS, resolveActiveElevationMechanics, resolveActiveLineOfSightMechanics } from "../content/elevation-mechanics.js";
...
export class TowerDefenseGame {
    ...
    applyAbilityEffect(enemy, effect, abilityId) {
    ...
    useAbility(abilityId, center) {
        ...
        for (const enemy of targets) {
            for (const effect of effects) {
                this.applyAbilityEffect(enemy, effect, abilityId);
            }
    ...
    static validateCheckpointState(content, identity, state) {
        ...
        const stringEventFields = new Set([
            ...
            "triggerDamageType", "budget"
        ]);
        const coordEventFields = new Set(["coord", "from", "to", "center", "originCoord"]);
    ...
    stabilizeDynamicEnemyNavigation() {
        ...
        for (const enemy of this.enemies) {
            ...
            if (this.navigationEnemyFields?.get(enemy.id) === field)
                continue;
            const lookup = this.navigationFieldLookupCache?.get(field);
    ...
    updatePulseTower(tower, pulseRate, pulseDamage, dotDuration, dotDamagePerUnit) {
        ...
        while (tower.cooldown <= 0 && pulses < 3) {
            let targets = this.enemies.filter((enemy) => enemy.hp > 0 && this.enemyTargetClass(enemy) === "ground" && this.enemyInRange(tower, enemy, this.towerRange(tower)));
    ...
    createNavigationResolver(occupiedCoords = this.navigationOccupiedCoords()) {
        const profile = this.activeNavigationProfile;
        if (!profile)

...
- Construct a minimal child environment. Linux uses a private runtime `HOME` and Windows uses a private profile. On macOS preserve the validated absolute login `HOME` only because the official Codex and Claude runtimes require it to discover the provider-owned login Keychain; keep `CODEX_HOME`, `CLAUDE_CONFIG_DIR`, the runtime working directory, and tools isolated. Do not inherit provider API keys, cloud credentials, proxy credentials, tracing, debug, or telemetry configuration. Disable nonessential runtime traffic and auto-updates where supported.

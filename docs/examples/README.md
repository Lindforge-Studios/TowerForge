...
| Opt-in hero roster and movement | `docs/examples/opt-in-hero-roster/` | `mechanics.json` is the static R5.1A `heroes` v1 reference; `mechanics-mobile.json` is the independent R5.1B v2 movement reference with heroes-owned flow profile, exact GameCommand v4, and no navigation activation. |

...
- Cross-platform v0.6.0 publication remains a delivery action until the tag, all six installers,
  `SHA256SUMS`, repeated release-note hashes and tagged source links are publicly verified.
...
| P3 | Signed distribution | v0.6.0 remains an unsigned pre-release. Developer ID notarization, Windows signing, store submission, hosted auth/matchmaking, and TowerForge Cloud need external deployment decisions and credentials. | Deployment milestone |

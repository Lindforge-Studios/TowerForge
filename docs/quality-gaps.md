...
- R15 is complete on its opt-in development branch. Its frozen tree passed repository, browser,
  plugin and package gates plus independent Code Verifier and Constructor Integration Verifier
  sign-offs; merge and release remain separate delivery actions.
...
| P3 | Signed distribution | v0.5.2 remains an unsigned pre-release. Developer ID notarization, Windows signing, store submission, hosted auth/matchmaking, and TowerForge Cloud need external deployment decisions and credentials. | Deployment milestone |

R14 and R15 are implemented as independent opt-in capabilities. R16–R17 remain paused; their
planned contracts MUST NOT appear in schema descriptors, recipes, Studio controls, player bundles,
or agent claims.

...
packages/player-shell (R21)
  one browser-only semantic DOM shell over detached HUD/action plans; no gameplay rules,
  Node/filesystem/native bridge, or world projection
...
R12.1 adds `enemyBehaviors` v1 as one such mission-selected profile. It may define at most 32
stable targetable component IDs per boss and optional tower `priorityTags`. The engine alone owns
root-to-component routing, component armor/shield/HP mutation, typed ability suppression, cleanup,
and optional snapshot/checkpoint inner schema v1. Canvas/Phaser consume one shared fail-closed
presentation projection; Studio and MCP author only through the existing guarded mechanics
transaction. An absent, disabled, unsupported, or unselected module adds no state, UI, player work,
or replay-digest change. See [ADR 0053](adr/0053-r12-advanced-enemy-behaviors.md).

R12.2 reuses TowerScript schema v7 rather than increasing any version domain. The typed
`bossComponentDamaged` and `bossComponentDestroyed` events expose an exact post-resolution,
event-scoped read-only `component` root to HFSM expressions. Existing leaf-to-ancestor transition
resolution, action budgets, Graph v2 handler/state/transition grammar, layout v1, and Trace/Debugger
v2 remain authoritative. Trace links component event records to transition provenance; no new
component action, Graph node kind, broad write tool, snapshot field, or checkpoint version exists.

R12.3 adds optional bounded formation cohorts to `enemyBehaviors` v1 and depends on an explicit
same-mission Navigation v1 `dynamic_flow` selection. Engine-owned spatial buckets pass at most 16
binary-ordered same-cohort neighbours to a pure selector over equal-optimal shared-flow candidates;
there is no per-enemy A*, O(n²) scan, renderer steering, or RNG. Active runtime state publishes only
`enemyBehaviors.formations` inner schema v1 with authoritative enemy/cohort/role assignments and
checkpoints that exact section. Canvas/Phaser consume one shared fail-closed projection. Inactive
projects keep the prior snapshot/checkpoint/player path.

R12.4 adds optional vanguard protection to those cohorts without creating a second shield or
damage system. The same mission must select Navigation v1 `dynamic_flow`, an active formation, and
Combat with a root Combat shield for the vanguard type. The engine searches at most 16 stable
same-cohort candidates and allows at most 512 redirects per public tick. Interception is one-hop;
the redirected packet then uses the common resolver and cannot recursively redirect again. Active
metadata lives only in `snapshot.enemyBehaviors.formations.protection`, while
`vanguardDamageIntercepted` is a read-only GameEvent and not a TowerScript event. Canvas/Phaser may
project the event but never pick a vanguard. An absent, disabled, unsupported, or unselected
dependency restores the prior path with no protection snapshot/checkpoint/player work.

R13.1 adds `ballistics` v1 as an independent mission-selected profile. Only explicitly bound,
unchained `single` tower attacks use authoritative projectiles; every absent, disabled, unselected,
unbound, or unsupported path retains immediate legacy damage. The engine fixes the target point,
endpoint elevations, component identity, and detached source-side `DamagePacket` at launch. It owns
fixed-tick travel, scalar altitude, miss detection, impact-time target defenses, active-only
snapshot/checkpoint inner v1, and stable digest/replay behavior. The shared renderer projection
copies and validates authoritative state, interpolates only screen coordinates, and contains no
collision, clearance, ricochet, weather, targeting, or damage rules. See
[ADR 0054](adr/0054-r13-deterministic-2-5d-ballistics.md).

R13.2 keeps the public Ballistics snapshot at v1 and adds optional tag-based launch-time clearance.
Profiles without clearance preserve checkpoint inner v1; selected clearance uses inner v2 with
captured immutable blocker provenance. The engine alone walks the topology line, combines effective
elevation with authored tag heights, enforces ray/inspection/terminal budgets, and emits
`projectileBlocked`. Shared renderers only validate and display that event.

R13.3 keeps the same module and public snapshot versions. Closed `terrainTags` and `armorTypes`
catalogs opt individual surfaces into ricochet, while each tower independently declares bounded
`maxBounces` and `rangeCells`. The engine topology registry owns the reflected ray and deterministic
candidate tie-break; renderer, Studio, and generated-player code consume only the fail-closed
`projectileRicocheted` projection. Ricochet-enabled state uses checkpoint inner v3. Removing the
ricochet catalogs/binding restores the R13.1/R13.2 path without adding surface UI or replay state.

R13.4 adds optional destructible map-object definitions and placements to Ballistics v1. The engine
owns HP/armor, projectile collision through the shared damage boundary, exactly-once destruction,
and any resulting persistent terrain/elevation candidate. A candidate is committed only after the
shared navigation contract proves every authored obligation remains reachable; rejection restores
the complete prior state. Active destructibles publish Ballistics snapshot v2 and checkpoint inner
v4 plus read-only damage/destruction events. Studio/MCP use a narrow five-file
preview/apply transaction with one revision, validation, backup, and rollback; renderers consume a
shared fail-closed projection. R13.4 adds no TowerScript action/event, automatic debris damage, or
implicit Procedural Juice binding. See `docs/examples/opt-in-destructible-environment/`.

R13.5 introduces a separate `weather` v1 mission capability rather than extending Ballistics.
The engine normalizes closed `{zones,definitions,schedule}` profiles, validates `all_map | tiles`
membership against the selected map, samples a canonical per-wave choice through the independent
...
terrain mutation, Ballistics coupling or automatic Procedural Juice cues. See the R13.5 amendment
in [ADR 0054](adr/0054-r13-deterministic-2-5d-ballistics.md) and
`docs/examples/opt-in-weather/`.

R14 introduces `arsenal` v1 as another independent mission-selected capability. Closed base,
barrel and core definitions are combined only by the pure engine compiler; it owns compatibility,
footprint and effective damage/range/durability. `GameCommandV7 configureTowerModules` and
`craftGem` are accepted only during setup or between waves and remain deterministic through journal
v7 and checkpoint restore. Gems are existing artifact instances: exact 3×3 recipes atomically
consume concrete unsocketed IDs and never create a second inventory/socket system. CampaignRun v1
imports explicitly to V2 with an empty `arsenal.moduleInventory`; `PlayerProfileV3`, project v3,
outer checkpoint and simulation versions remain unchanged. Only active missions publish
`snapshot.arsenal` v1. Studio and Canvas/Phaser players use the shared fail-closed renderer
projection and submit commands; MCP uses inert recipes plus the existing guarded mechanics
transaction. See [ADR 0055](adr/0055-r14-modular-arsenal.md) and
`docs/examples/opt-in-modular-arsenal/`.

R15 adds `macroEconomy` v1 as an independent mission-selected capability. The engine alone owns
the domain-separated seeded price step, current-wave net demand, commodity holdings, cleared-wave
deposit maturity and atomic ritual preflight/effects. Exact `GameCommandV8` buy/sell/deposit/ritual
commands promote journals to v8 while replay continues to accept v1–v8. Active-only
`snapshot.macroEconomy` and optional checkpoint `macroEconomy` v1 are rejected when the capability
is absent, and restore validates authored IDs, exact record coverage, quote bounds and seed
provenance, including deposit open/maturity boundaries and exact ritual effect origin. Co-op v1
requires shared resources and applies `owner_only` to every sacrificed tower. Studio, MCP and
Canvas/Phaser consume descriptors and the fail-closed renderer
projection; they never derive prices, eligibility, interest or combat effects. The existing
...
`content/visuals.json` schema v2 owns `assetsRoot`, atlases, standalone/frame sprites, weighted tile variants, Wang/signature materials, map/grid tileset bindings, event SFX, looping music tracks, and per-mission music. Schema v3 is an explicit storage-compatible extension with optional closed `proceduralJuice` v1 catalogs for deterministic particles, oscillator audio, camera cues, and event bindings. R20 adds visuals v4 as a second independent storage-compatible extension with bounded `CameraProfileV1`, mission/map bindings and projection/orientation view variants. Resolution is mission -> map -> BuildTargets-v2 target -> built-in top-down. Absence keeps the literal v2/v3 top-down path unchanged. Asset paths are project-relative only. R21 references those existing sprite IDs from its separate optional `content/hud.json`; the HUD catalog never embeds binaries, URLs, host paths, CSS, HTML, JavaScript, fonts or SVG.

### R21 HUD catalog and shell

R21 is accepted and published in `v0.8.0` after exact-commit gates and both independent sign-offs.
`content/hud.json` stores closed data-only `HudCatalogV1`; it becomes active only through
...
The independent `@towerforge/engine/replay-lab` entrypoint builds on the existing checkpoint,
journal and replay contracts without changing simulation state. `ReplayArchiveV1` uses the fixed
`TFRP` v1 binary envelope, 20-byte big-endian header, domain-separated checksum/digests and a hard
72 MiB archive ceiling. Decode rejects malformed header, length, checksum, non-canonical payload,
unsupported identity and incompatible content/capabilities before journal execution. A decoded
archive can create a read-only Ghost session with detached frozen frames and at most 256 cached
frames, or an immutable `ReplayBranchV1` rooted at an exact parent sequence. Branch replay owns a
new journal suffix and reports the first command/state divergence without mutating source bytes.

Replay Lab is an explicitly opened Studio-only inspection surface, not a project mechanics module.
Its MCP tools are compute-only and write no project files or network state. Ordinary generated
players and PWA/single-file/mobile/desktop carriers omit the Replay Lab entrypoint and ghost
projector. The separate `@towerforge/reference-relay` package reuses the R8 capability handshake
and only forwards bounded opaque FIFO frames in invite-code rooms. It accepts injected network
ports, defaults its server adapter to loopback, and contains no simulation, accounts, auth or
matchmaking. See [ADR 0057](adr/0057-r16-ghost-replay-lab.md).
...
`packages/mcp/agent-instructions.mjs` is the canonical mechanism-selection and safe-workflow policy for Studio direct APIs, Codex, Claude, and external MCP initialization. `describe_schema({domain})` progressively exposes implemented domain descriptors, including `director`, `enemyBehaviors`, `ballistics`, `weather`, `arsenal`, `macroEconomy`, the presentation-only `proceduralJuice`, and the compute-only `replayLab`, while `get_capabilities` is read-only and never creates the optional mechanics catalog or upgrades a project. Preview/apply reject unavailable modules, unsupported versions, and unmet dependencies before any write. Executable mechanics modules include combat v1–v3; reactions/navigation/physics/terraforming/director/quests/enemyBehaviors/ballistics/weather/arsenal/macroEconomy v1; elevation v1–v3; roguelite v1–v4; heroes v1–v7; logistics v1–v3; and multiplayer v1–v2. Dependencies and promotions remain explicit; mechanics recipes are detached candidates and the common guarded mechanics transaction owns the catalog/mission write. Destructibles use their narrower multi-file preview/apply transaction. Procedural Juice instead uses its own combined project+visuals revision preview/apply transaction; its recipes and event preview are compute-only. Replay Lab tools inspect, verify and branch-analyze supplied archives without project writes or sockets. The stdio server queues frames FIFO, drains stdout before normal exit, and keeps one controlled lifetime error path for late broken pipes.
...
- Mechanics Hub: a separate opt-in workspace for engine-owned module cards and capability state,
  including the Director authored-counter editor and R12 targetable-boss component JSON editor;
  unavailable or unselected mechanics do not add
...
The player imports `./engine/index.js` and `./player-runtime/index.mjs`, runs `TowerDefenseGame` in the browser, and renders with Canvas or Phaser. Before either entrypoint executes, official builds show the inline system-owned `Made with TowerForge` splash. It remains the immutable first surface, yields immediately to boot recovery on failure, makes no network request, and is not project/HUD data. R22 optionally lets one BuildTargets-v2 target select `SplashCatalogV1` from `content/splashes.json`; one to eight validated local image frames then follow the engine credit while runtime boot proceeds in parallel. Runtime readiness and playlist completion are separate barriers, and `BootOk` is published only when both system/project layers are hidden. Unbound targets do not parse or ship the catalog. Therefore PWA, single-file, portable web, Capacitor and generated Tauri carriers receive the same engine credit and active-only project playlist without a separate packaging rule. Official authoring/MCP cannot remove the engine surface; MIT source forks may alter their own compiler. See [ADR 0063](adr/0063-r22-project-splash-playlists.md). Both renderer templates use the same immutable profile reducers and persistence fragment for difficulty, meta currencies/upgrades/rewards, campaign unlocks, warnings, and reset behavior. The build precaches the runtime for PWA use and embeds each relative module once in a flat `index.single.html` registry; a tiny bootstrap materializes dependency-ordered Blob URLs so graph growth stays linear and direct `file://` launch remains valid. Plugin and desktop runtime preparation mirror the same package.
...
The staged R0–R22 program, TDD roles, forbidden increment combinations, compatibility baseline, and delivery status live in [ROADMAP.md](ROADMAP.md). Accepted R0–R8 establish opt-in mechanics, the common damage/modifier boundary, deterministic checkpoints/journals/profile state, navigation/elevation/terraforming, roguelite runs, heroes/logistics, TowerScript DX 2.0, generative authoring, and local multiplayer. Accepted R9 adds script-local TowerScript v7 Behavior Tree/HFSM controllers plus v2 Graph/Trace/Debugger surfaces. Accepted R10 adds compute-only Persona QA and mission-selected procedural quests. Accepted R11 independently layers deterministic JSON particle/audio/camera presentation over the authoritative event surface without changing gameplay state. Accepted R12 adds targetable boss components, schema-v7 component-driven phases, bounded formation steering and vanguard shield interception. Accepted R13 adds deterministic projectiles, arc clearance, bounded ricochet, transactional destructibles and independent seeded Weather. Accepted R14 adds CampaignRunV2, opt-in modular tower assembly and atomic gem crafting. Accepted R15 adds the opt-in deterministic local market, deposits and rituals. Accepted R16 adds isolated Replay Lab and reference-relay boundaries. Accepted R17 adds opt-in Distribution v1, reproducible publish manifests, explicit-confirm provider adapters, deterministic Remix source packs and host-only monetization. Accepted R18 adds the large-screen web player, accepted R19 adds the first-class generated native carrier, and accepted R20 adds presentation-only camera profiles plus one shared renderer projector. Accepted R21 adds an explicitly bound data-only HUD catalog, pure layout/screen-graph/menu runtimes, guarded Studio/MCP authoring, conditional package parity and one browser DOM shell. Accepted R22 adds build-target-selected project splash playlists after the immutable TowerForge system splash. The combined R0–R21 baseline is published as `v0.8.0`; R0–R22 form the `v0.9.0` source candidate until release verification completes.

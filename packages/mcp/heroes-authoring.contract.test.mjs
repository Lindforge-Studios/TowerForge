...
describe("R5.4A MCP/AI battle-local hero skill-tree authoring", () => {
  ...
  it("keeps the guarded tree flow in guide v29 with the authoritative v6 command contract", () => {
    expect(TOWERFORGE_AGENT_GUIDE_VERSION).toBe(29);
...
describe("R5.5A MCP/AI passive hero damage-aura authoring", () => {
  ...
  it("keeps explicit aura promotion and membership in guide v29", () => {
    expect(TOWERFORGE_AGENT_GUIDE_VERSION).toBe(29);
    expect(TOWERFORGE_AGENT_INSTRUCTIONS).toMatch(
...
describe("R5.6A MCP/AI dynamic-navigation hero blocking authoring", () => {
  ...
  it("publishes guide v29 with explicit dependency, promotion, and snapshot-only presentation", () => {
    expect(TOWERFORGE_AGENT_GUIDE_VERSION).toBe(29);
    expect(TOWERFORGE_AGENT_INSTRUCTIONS).toMatch(

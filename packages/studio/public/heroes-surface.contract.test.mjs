...
describe("R5.1A Studio static heroes foundation surface", () => {
  it("describes the Heroes card as a static core roster with active control deferred", () => {
    ...
    const copy = match?.[1] ?? "";
    expect(copy).toMatch(/static/i);
    expect(copy).toMatch(/roster/i);
    expect(copy).toMatch(/core/i);
    expect(copy).toMatch(/later[\s-]+active[\s-]+control/i);
  ...
  it("keeps the exact roster editor isolated inside Mechanics Hub", () => {
    ...
    expect(editor).not.toMatch(/mana|ability|skill|aura|blocking|move speed|shield|TowerScript/i);
  });

  it("normalizes only own bounded definitions and saves the whole profile via revision guard", () => {
    ...
    expect(normalize).toMatch(/selectedHeroId/);
    expect(normalize).toMatch(/definitions/);
    expect(normalize).toMatch(/spawn/);
    expect(normalize).toMatch(/core/);
    expect(normalize).toMatch(/32|definitions/);
    expect(normalize).toMatch(/128|idUtf8Bytes|labelUtf8Bytes/);
  ...
  it("enables v1, preserves it across disable/re-enable, and keeps future v2 read-only", () => {
    ...
    expect(app).toMatch(/heroes[\s\S]{0,500}(?:future|read-only|schemaVersion\s*2)/i);
    expect(app).not.toMatch(/schemaVersion\s*:\s*4[\s\S]{0,120}moveHero|type\s*:\s*["']moveHero["']/);
  });
});

...
interface EvaluationState {
  readonly context: TowerScriptBehaviorTreeContextV1;
  readonly candidates: readonly TowerScriptTargetCandidateV1[];
  readonly visited: string[];
  readonly trace: TowerScriptBehaviorNodeTraceV1[];
  readonly expressionBudget: { remaining: number };
...
function compareCandidates(mode: TowerTargetMode, left: TowerScriptTargetCandidateV1, right: TowerScriptTargetCandidateV1): number {
  let result = 0;
  if (mode === "last") result = left.routeProgress - right.routeProgress;
  else if (mode === "closest") result = left.distance - right.distance;
  else if (mode === "furthest") result = right.distance - left.distance;
  ...
  } else result = right.routeProgress - left.routeProgress;
  return result || compareBinary(left.id, right.id);
}

function expressionRoot(
  state: EvaluationState,
  candidate: TowerScriptTargetCandidateV1 | null
...
function evaluateNode(node: TowerScriptBehaviorNodeV1, state: EvaluationState, depth: number): boolean {
  ...
  const finish = (success: boolean, selectedTargetIds?: readonly string[]): boolean => {
    if (!success) state.selected = selectedBeforeNode;
  ...
  if (node.type === "selector") {
    for (const child of node.children) if (evaluateNode(child, state, depth + 1)) return finish(true);
    return finish(false);
  }
  if (node.type === "sequence") {
    for (const child of node.children) if (!evaluateNode(child, state, depth + 1)) return finish(false);
    return finish(true);
  }
  ...
  if (node.type !== "action" || node.action !== "select_targets"
    || !(TOWER_TARGET_MODES as readonly string[]).includes(node.mode)) {
    throw new Error("TowerScript behavior tree action is invalid.");
  }
  const selected = node.filter === undefined
    ? [...state.candidates]
    : state.candidates.filter((candidate) => Boolean(evaluateTowerScriptExpression(
        node.filter!,
        expressionRoot(state, candidate),
        state.expressionBudget
      )));
 ...
 */
export function evaluateTowerScriptBehaviorTree(
  tree: TowerScriptBehaviorTreeV1,
  context: TowerScriptBehaviorTreeContextV1
): TowerScriptBehaviorTreeResultV1 {

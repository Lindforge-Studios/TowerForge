...
export const REPLAY_ARCHIVE_SCHEMA_VERSION = 1 as const;
export const REPLAY_ARCHIVE_MAGIC = Object.freeze([0x54, 0x46, 0x52, 0x50] as const);
export const REPLAY_ARCHIVE_HEADER_BYTES = 20 as const;
...
const decodedArchiveContent = new WeakMap<object, GameContentRegistry>();
...
export function replayArchiveContentV1(archive: unknown): GameContentRegistry | undefined {
  return archive !== null && typeof archive === "object" ? decodedArchiveContent.get(archive) : undefined;
}

function consumeHashBytes(hash: bigint, bytes: Uint8Array): bigint {
  let current = hash;
...
function domainHash(domain: string, bytes: Uint8Array): bigint {
  return consumeHashBytes(consumeHashBytes(FNV1A_64_OFFSET, textEncoder.encode(domain)), bytes);
}

function domainDigest(prefix: string, domain: string, bytes: Uint8Array): string {
...
function ownedArchiveBytes(value: unknown): Uint8Array {
...
export function encodeReplayArchiveV1(options: {
  ...
  const envelope: ReplayArchiveEnvelopeV1 = {
...
export function decodeReplayArchiveV1(options: {
  ...
  for (let index = 0; index < REPLAY_ARCHIVE_MAGIC.length; index += 1) {
    if (bytes[index] !== REPLAY_ARCHIVE_MAGIC[index]) throw new Error("Replay archive magic is invalid.");
  }
  if (bytes[4] !== REPLAY_ARCHIVE_SCHEMA_VERSION) throw new Error("Replay archive version is unsupported.");
  if (bytes[5] !== ARCHIVE_FLAGS_V1) throw new Error("Replay archive flags are unsupported.");
  ...
  const journal = decodeGameCommandJournal({
    ...
    journal: dataValue(descriptors, "journal") as GameCommandJournal
  });
  ...
  const archive = Object.freeze({
    schemaVersion: REPLAY_ARCHIVE_SCHEMA_VERSION,
    engineVersion: SIMULATION_ENGINE_VERSION,
    payloadKind: "game_command_journal" as const,
    contentDigest,

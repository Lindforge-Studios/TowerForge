...
import {
  ...
  type ResolvedWorldCampaign
} from "./campaign-world.js";

export type CampaignBattleFailureCode =
  | "campaign_inactive"
...
function isBattleNode(node: unknown): node is WorldCampaignBattleNodeV1 {
  return Boolean(
    node
    && typeof node === "object"
    && "type" in node
...
function canonicalCampaignLaunchGraph(campaign: ResolvedWorldCampaign): unknown {
  return {
    ...campaign,
...
function battleDraftChoiceCount(content: GameContentRegistry, node: WorldCampaignNodeV1 | WorldCampaignNodeV2): number {
  if (!isBattleNode(node)) return 0;
  const active = resolveActiveRogueliteMechanics(content, node.missionId);
  if (!active?.draft) return 0;
  return Math.max(0, (content.missions[node.missionId]?.waves.length ?? 0) - 1);
}

function maximumRemainingDraftChoices(
  content: GameContentRegistry,
  ...
  if (!node) return 0;
  if (includeNode) return visit(nodeId);
...
function prepareValidatedCampaignBattle(
  validation: Extract<CampaignRunContentValidationResult, { ok: true }>,
  content: GameContentRegistry,
  nodeId: string,
  construct: boolean
): CampaignBattlePreparationResult {
  const captured = validation.run;
  const fail = (code: CampaignBattleFailureCode): CampaignBattlePreparationResult => Object.freeze({
    ok: false as const,
    code,
    run: captured
  });
  if (!availableNodeIds(captured, validation.campaign).includes(nodeId)) return fail("node_not_available");
  ...
  const requiredDraftEntries = maximumRemainingDraftChoices(
    content,
    validation.campaign.nodes,
    nodeId,
    true
  );
  if (
  ...
  const maxNewArtifactInstances = Math.min(
    CAMPAIGN_RUN_LIMITS.collectionEntries
      ...
      - Object.keys(captured.runResources).length
      - requiredDraftEntries,
    CAMPAIGN_RUN_LIMITS.collectionEntries - captured.artifacts.length
  );
  ...
  const loadout: CampaignBattleLoadoutV1 = Object.freeze({
    schemaVersion: 1,
    launchId: id,
    nodeId,
  ...
  const launch: CampaignBattleLaunchV1 = Object.freeze({
    launchId: id,
    battleSeed: seed,
  ...
  const game = construct
    ...
    : undefined;
  if (!game) return fail("battle_context_mismatch");
  return Object.freeze({
    ok: true as const,
    code: "campaign_battle_prepared" as const,
    nodeId,
    missionId: node.missionId,
    launchId: id,
    battleSeed: seed,
...
export function prepareCampaignBattle(
  ...
  if (!validation.ok) return Object.freeze({ ok: false, code: validation.code, run: validation.run });
...
export function settleCampaignBattleVictory(
  ...
  const validation = validateCampaignRunAgainstContent(run, content);
  const captured = validation.run;
  const fail = (code: CampaignBattleFailureCode): CampaignBattleSettlementResult => Object.freeze({
    ok: false as const,
    code,
    run: captured,
    profile
  ...
  if (!isBattleNode(node)) return fail("node_type_not_implemented");
  const active = resolveActiveRogueliteMechanics(content, node.missionId);
  if (active?.schemaVersion !== 4 || active.campaign?.schemaVersion !== 2) return fail("campaign_handoff_inactive");
  ...
  if (!settlement) return fail("battle_not_victorious");
  if (
    settlement.launchId !== expectedLaunchId
  ...
  const remainingDraftEntries = maximumRemainingDraftChoices(
    content,
    validation.campaign.nodes,
    nodeId,
    false
  ...
  return Object.freeze({
    ok: true as const,
    code: "campaign_battle_settled" as const,
    nodeId,
    run: nextRun,

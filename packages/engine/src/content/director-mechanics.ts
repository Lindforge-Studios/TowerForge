...
function closed(record: Record<string, unknown>, allowed: readonly string[], path: string): void {
  const allow = new Set(allowed);
  for (const key of Object.keys(record)) {
    if (!allow.has(key)) throw new DirectorProfileValidationError(`${path} is closed; unknown field "${key}".`);
  }
...
function plainArray(value: unknown, path: string, max: number): readonly unknown[] {
...
function stringValue(value: unknown, path: string, maxBytes: number): string {
...
function normalizeCondition(value: unknown, path: string): DirectorConditionV1 {
  ...
  if (typeof metric !== "string" || !DIRECTOR_METRICS.includes(metric as DirectorMetricV1)) {
  ...
  return Object.freeze({
    metric: metric as DirectorMetricV1,
...
function normalizeGroup(value: unknown, path: string): WaveGroup {
  const record = dataRecord(value, path);
...
export function normalizeDirectorProfileV1(value: unknown): DirectorProfileV1 {
  ...
  const counterPool = Object.create(null) as Record<string, DirectorCounterV1>;
  let totalGroups = 0;
  for (const counterId of counterIds) {
    ...
    if (conditions.length === 0) throw new DirectorProfileValidationError(`Director counter "${counterId}" requires a condition.`);
    const groups = plainArray(raw.groups, `director profile.counterPool.${counterId}.groups`, DIRECTOR_LIMITS.groupsPerCounter);
    if (groups.length === 0) throw new DirectorProfileValidationError(`Director counter "${counterId}" requires a group.`);
...
export function resolveActiveDirectorMechanics(
  content: GameContentRegistry,
  missionId: string
): ActiveDirectorMechanicsV1 | undefined {

import {
  ...
  type LogisticsTowerInventoryDefinitionV2
} from "../content/logistics-mechanics.js";
import type {
  LogisticsAmmunitionInventorySnapshotV2,
  TowerState
} from "./types.js";
...
export function isLiveLogisticsAmmunitionTower(tower: Pick<TowerState, "hp">): boolean {
  return tower.hp === undefined || tower.hp > 0;
}

export function isAmmunitionBoundTowerType(
...
export function getLogisticsAmmunitionTowerInventory(
  ammunition: LogisticsAmmunitionDefinitionV2,
  towerTypeId: string
): LogisticsTowerInventoryDefinitionV2 | undefined {
  return isAmmunitionBoundTowerType(ammunition, towerTypeId)
    ? ammunition.towerInventories[towerTypeId]
    : undefined;
}

export function assertLogisticsAmmunitionPlacement(
  ammunition: LogisticsAmmunitionDefinitionV2,
  towers: readonly Pick<TowerState, "typeId" | "hp">[],
  candidateTypeId: string
): void {
  if (!isAmmunitionBoundTowerType(ammunition, candidateTypeId)) return;
  let live = 1;
  for (const tower of towers) {
    if (isLiveLogisticsAmmunitionTower(tower)
      && isAmmunitionBoundTowerType(ammunition, tower.typeId)) live += 1;
    if (live > LOGISTICS_AMMUNITION_LIMITS.liveInventories) {
      throw new Error(
        `Logistics ammunition live inventory limit ${LOGISTICS_AMMUNITION_LIMITS.liveInventories} exceeded.`
      );
    }
  }
}

export function buildLogisticsAmmunitionSnapshotV2(
  ammunition: LogisticsAmmunitionDefinitionV2,
  towers: readonly TowerState[],
  amounts: ReadonlyMap<string, number>
): Readonly<{ readonly inventories: readonly LogisticsAmmunitionInventorySnapshotV2[] }> {
  const inventories = towers
    ...
    .map((tower): LogisticsAmmunitionInventorySnapshotV2 => {

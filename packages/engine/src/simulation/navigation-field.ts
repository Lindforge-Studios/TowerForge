...
interface NormalizedRequest {
  ...
  readonly goal: GridCoord;
  readonly profile: MovementProfileV1;
  readonly terrainWalkability: ReadonlyMap<string, boolean>;
  readonly terrainByCoord: ReadonlyMap<string, string>;
...
function denseArray(value: unknown, context: string, maxLength?: number): readonly unknown[] {
  let prototype: object | null;
  try {
    prototype = value !== null && typeof value === "object" ? Object.getPrototypeOf(value) : null;
  } catch {
  ...
  let earlyLengthDescriptor: PropertyDescriptor | undefined;
  try {
  ...
  const earlyLength = earlyLengthDescriptor.value as number;
  ...
  if (length !== earlyLength) return fail(`${context} array length changed during inspection.`);
...
function positiveSafeInteger(value: unknown, context: string, maximum: number): number {
...
function terrainCost(value: unknown, context: string): number | null {
...
function normalizeProfile(value: unknown): MovementProfileV1 {
  ...
  const defaultTerrainCost = terrainCost(
    required(profile, "defaultTerrainCost", "profile"),
    "profile.defaultTerrainCost"
  );
...
function normalizeTerrainTypes(value: unknown): ReadonlyMap<string, boolean> {
  ...
  if (terrainIds.length > NAVIGATION_FIELD_INPUT_LIMITS.terrainDefinitions) {
    fail(
      `terrainTypes definition count exceeds the `
  ...
  for (const terrainId of terrainIds.sort(compareBinary)) {
    ...
    if (
      ...
      fail(
        `terrainTypes.${terrainId}.label must contain `
    ...
    if (typeof walkable !== "boolean") fail(`terrainTypes.${terrainId}.walkable must be boolean.`);
    ...
    for (let tagIndex = 0; tagIndex < tags.length; tagIndex += 1) {
      ...
      if (typeof tag !== "string") fail(`terrainTypes.${terrainId}.tags must contain strings.`);
      if (utf8ByteLength(tag) > NAVIGATION_FIELD_INPUT_LIMITS.terrainTagUtf8Bytes) {
        fail(
          `terrainTypes.${terrainId}.tags[${tagIndex}] exceeds the `
...
function parseCoordKey(value: string, width: number, height: number, context: string): GridCoord {
  const match = /^(0|[1-9]\d*),(0|[1-9]\d*)$/.exec(value);
  if (!match) return fail(`${context} key "${value}" must use canonical q,r coordinates.`);
...
function normalizeTerrainGrid(
  value: unknown,
  width: number,
  height: number,
  terrainTypes: ReadonlyMap<string, boolean>
  ...
  for (let r = 0; r < height; r += 1) {
    for (let q = 0; q < width; q += 1) {
      if (!result.has(`${q},${r}`)) fail(`terrainByCoord is missing cell ${q},${r}.`);
    }
  }
  return result;
}

function normalizeOccupiedCoords(value: unknown, width: number, height: number): ReadonlySet<string> {
  const authored = denseArray(value, "occupiedCoords");
  if (authored.length > width * height) fail("occupiedCoords exceeds the map cell budget.");
...
function normalizeBudget(value: unknown): { maxCells: number; maxRelaxations: number } {
  if (value === undefined) {
...
function normalizeRequest(value: unknown): NormalizedRequest {
  ...
  const width = positiveSafeInteger(
    required(request, "width", "request"),
    "width",
    NAVIGATION_LIMITS.activeMapCells
  );
  const height = positiveSafeInteger(
    required(request, "height", "request"),
    "height",
    NAVIGATION_LIMITS.activeMapCells
  );
  ...
  if (!Number.isSafeInteger(cellCount) || cellCount > NAVIGATION_LIMITS.activeMapCells || cellCount > budget.maxCells) {
    fail(
      `map dimensions contain ${cellCount} cells, exceeding the ${budget.maxCells} cell budget `
  ...
  return {
    ...
    width,
    height,
    movementProfileId: boundedId(
      required(request, "movementProfileId", "request"),
      "movementProfileId"
    ),
    ...
    terrainByCoord: normalizeTerrainGrid(
      required(request, "terrainByCoord", "request"),
      width,
      height,
      terrainTypes
    ),
    occupiedKeys: normalizeOccupiedCoords(
...
class MinQueue {
  ...
  push(entry: QueueEntry): void {
    ...
    while (index > 0) {
      ...
      if (compareQueue(parentEntry, entry) <= 0) break;
  ...
  pop(): QueueEntry | undefined {
    ...
    while (true) {
      ...
      if (compareQueue(tail, child) <= 0) break;
...
function inside(coord: GridCoord, request: NormalizedRequest): boolean {
  return coord.q >= 0 && coord.q < request.width && coord.r >= 0 && coord.r < request.height;
}

function effectiveCost(coord: GridCoord, request: NormalizedRequest): number | null {
  const key = coordKey(coord);
  if (request.profile.towerOccupancy === "blocked" && request.occupiedKeys.has(key)) return null;
  ...
  if (hasOverride) return request.profile.terrainCosts![terrainId] ?? null;
...
function directionIndex(from: GridCoord, to: GridCoord, neighbors: readonly GridCoord[]): number {
  const index = neighbors.findIndex((candidate) => candidate.q === to.q && candidate.r === to.r);
  return index < 0 ? Number.MAX_SAFE_INTEGER : index;
}

function prefersNext(
  origin: GridCoord,
  candidate: GridCoord,
  incumbent: GridCoord | undefined,
  neighbors: readonly GridCoord[]
): boolean {
...
function freezeCoord(coord: GridCoord): GridCoord {
 ...
 */
export function buildNavigationField(request: NavigationFieldRequest): NavigationFieldResult {
  const normalized = normalizeRequest(request);
  const topology = createGridTopology(normalized.grid);
  const goalKey = coordKey(normalized.goal);
  const distances = new Map<string, number>();
  const nextByCoord = new Map<string, GridCoord>();
  const coordsByKey = new Map<string, GridCoord>();
  ...
  for (let current = queue.pop(); current; current = queue.pop()) {
    ...
    if (distances.get(currentKey) !== current.distance) continue;
    ...
    for (const predecessor of topology.neighbors(current.coord)) {
      if (!inside(predecessor, normalized) || effectiveCost(predecessor, normalized) === null) continue;
      relaxations += 1;
      ...
      if (!Number.isSafeInteger(candidateDistance)) fail("distance addition exceeded the safe-integer range.");
  ...
  const cells = [...distances.entries()]
    .map(([key, distance]): NavigationFieldCell => {

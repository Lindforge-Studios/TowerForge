...
describe("R5.4A exact GameCommand/Journal v6 skill boundary (RED)", () => {
  it("accepts only the closed v6 unlock envelope and preserves older aliases", () => {
    expect((Engine as any).GAME_COMMAND_SCHEMA_VERSION).toBe(8);
    expect((Engine as any).GAME_COMMAND_SUPPORTED_SCHEMA_VERSIONS).toEqual([1, 2, 3, 4, 5, 6, 7, 8]);
    expect((Engine as any).GAME_COMMAND_JOURNAL_SCHEMA_VERSION).toBe(8);
    expect((Engine as any).GAME_COMMAND_JOURNAL_SUPPORTED_SCHEMA_VERSIONS).toEqual([1, 2, 3, 4, 5, 6, 7, 8]);

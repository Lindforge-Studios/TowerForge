import { resolveModifiers, type ModifierSpec, type ModifierTraceStep } from "./modifiers.js";
import {
  ARMOR_MATRIX_LIMITS,
  MARK_LIMITS,
  type MarkConsumePolicy
...
function normalizeMarks(value: unknown): readonly ActiveMarkDamageContext[] {
  if (value === undefined) return [];
  if (!Array.isArray(value)) throw new Error("Damage marks must be an array.");
  if (value.length > MARK_LIMITS.definitions) {
    throw new Error(`Damage marks exceed the maximum limit of ${MARK_LIMITS.definitions}.`);
  }
  const normalized: ActiveMarkDamageContext[] = [];
  ...
  for (let index = 0; index < value.length; index += 1) {
    ...
    if (seen.has(markId)) throw new Error(`Damage mark id "${markId}" is duplicated.`);
    seen.add(markId);
    const stacks = mark.stacks;
    if (!Number.isSafeInteger(stacks) || (stacks as number) <= 0 || (stacks as number) > MARK_LIMITS.maxStacks) {
    ...
    let damageTypes: readonly string[] | undefined;
    if (mark.damageTypes !== undefined) {
      if (
        !Array.isArray(mark.damageTypes)
...
function ownDataRecord(value: unknown, label: string, maximumEntries?: number): Record<string, unknown> {
...
function normalizeArmorMatrix(value: unknown): ArmorMatrixContext {
  ...
  const multipliers = ownDataRecord(
    matrix.multipliers,
    "Damage armor matrix multipliers",
    ARMOR_MATRIX_LIMITS.damageTypes
  );
  const normalizedMultipliers: Record<string, number> = Object.create(null) as Record<string, number>;
...
function normalizeResistances(
  resistances: Readonly<Record<string, number>> | undefined
): Readonly<Record<string, number>> | undefined {
  if (resistances === undefined) return undefined;
...
function validateTags(tags: readonly DamageTag[] | undefined): void {
  if (tags === undefined) return;
  if (!Array.isArray(tags)) {
    throw new Error("Damage tags must be an array.");
  }
  for (const tag of tags) {
    if (!(DAMAGE_TAGS as readonly unknown[]).includes(tag)) {
...
function validateSource(source: DamageSourceRef): void {
  ...
  switch (source.kind) {
    case "tower":
      ...
      if (source.towerId !== undefined) validateId(source.towerId, "Tower source towerId");
      return;
    case "ability":
    ...
    case "leak":
      ...
      return;
    default:
      throw new Error(`Unsupported damage source kind "${String((source as { kind?: unknown }).kind)}".`);
  }
}

function validateTarget(target: DamageTargetRef): void {
  ...
  switch (target.kind) {
    ...
    case "core":
      return;
    default:
      throw new Error(`Unsupported damage target kind "${String((target as { kind?: unknown }).kind)}".`);
  }
}
...
export class DamageResolver {
  static resolve(packet: DamagePacket, context: DamageResolutionContext = {}): DamageResolution {
    ...
    const modifierResolution = resolveModifiers(
      packet.amount,
      "damage",
      packet.modifiers === undefined ? [] : packet.modifiers
    );
    const damageType = packet.damageType ?? "physical";
    const marks = normalizeMarks(context.marks);
    const markTrace: MarkDamageTraceStep[] = [];
    let afterMarks = modifierResolution.value;
    for (const mark of marks) {
      if (mark.damageTypes !== undefined && !mark.damageTypes.includes(damageType)) continue;

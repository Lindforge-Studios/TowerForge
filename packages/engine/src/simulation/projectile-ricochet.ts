...
function inspectRecord(value: unknown, context: string): Readonly<Record<string, unknown>> {
  ...
  if (prototype !== Object.prototype) fail(`${context} must use the plain object prototype.`);
  if (Object.getOwnPropertySymbols(descriptors).length > 0) fail(`${context} rejects symbol fields.`);
...
function exactKeys(record: Readonly<Record<string, unknown>>, keys: readonly string[], context: string): void {
...
function coord(value: unknown, map: GridMap, context: string): GridCoord {
  ...
  if (!map.isInside(result)) fail(`${context} is outside the map.`);
  return result;
}
...
export function traceProjectileRicochetRayV1(
  map: GridMap,
  value: ProjectileRicochetRayRequestV1,
  remainingCellInspections: number = RICOCHET_LIMITS.cellInspectionsPerTick
): ProjectileRicochetRayResultV1 {
  const request = inspectRecord(value, "request");
  exactKeys(request, ["kind", "incomingFromCoord", "collisionCoord", "rangeCells"], "request");
  if (request.kind !== "terrain" && request.kind !== "armor") fail("kind must be terrain or armor.");
  ...
  if (!Number.isSafeInteger(request.rangeCells)
    || (request.rangeCells as number) < 1
    || (request.rangeCells as number) > RICOCHET_LIMITS.maximumReflectedRayDistance) {
  ...
  if (incomingDirection === undefined) fail("incoming and collision coordinates must be topology-adjacent.");
  ...
  let inspections = 0;
  for (let index = 0; index < (request.rangeCells as number); index += 1) {
    ...
    if (!next || !map.isInside(next)) break;
    inspections += 1;

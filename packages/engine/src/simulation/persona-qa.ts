import type { GameContentRegistry, MissionContentDefinition } from "../content/registry.js";
import { dispatchGameCommand, type GameCommandV6 } from "./commands.js";
...
import type { GameSnapshot, GridCoord, TowerType } from "./types.js";
...
function ownDataFields(value: unknown, label: string): DataFields {
  try {
...
function requireClosedFields(fields: DataFields, expected: readonly string[], label: string): void {
...
function normalizeRequest(input: unknown): PersonaQaRequestV1 {
  ...
  const personaIds = normalizeStringList(
    ...
    PERSONA_QA_LIMITS.personaIds,
    new Set(PERSONA_QA_PERSONA_IDS)
  ) as readonly PersonaQaPersonaId[];
  const simSeconds = normalizePositiveNumber(fields.get("simSeconds"), "simSeconds", 0.05, PERSONA_QA_LIMITS.simSeconds);
  const tickStep = normalizePositiveNumber(
    fields.get("tickStep"),
    "tickStep",
    PERSONA_QA_LIMITS.minimumTickStep,
    PERSONA_QA_LIMITS.maximumTickStep
  );
...
function canonicalRegistry(content: GameContentRegistry): GameContentRegistry {
  const towers = Object.fromEntries(
    Object.entries(content.towers).sort(([left], [right]) => compareBinary(left, right))
  );
  const missions = Object.fromEntries(
    Object.entries(content.missions)
      .sort(([left], [right]) => compareBinary(left, right))
      .map(([id, mission]) => [id, {
        ...mission,
        buildTowerIds: [...mission.buildTowerIds].sort(compareBinary)
      } satisfies MissionContentDefinition])
  );
  return { ...content, towers, missions };
}

function towerCost(type: TowerType): number {
  return Object.values(type.cost).reduce((sum, amount) => sum + (amount ?? 0), 0);
}

function attackPower(type: TowerType): number {
...
function economyYield(type: TowerType): number {
  if (type.attack.kind !== "pipeline") return 0;
  return type.attack.effects.reduce((total, effect) => {
    if (effect.kind !== "resource") return total;
    return total + Object.values(effect.resources).reduce((sum, amount) => sum + (amount ?? 0), 0);
  }, 0);
}

function rankTowerIds(
  content: GameContentRegistry,
  missionId: string,
  personaId: PersonaQaPersonaId
): readonly string[] {
...
function placementComparator(
  snapshot: GameSnapshot,
  personaId: PersonaQaPersonaId
): (left: GridCoord, right: GridCoord) => number {
  const distance = createGridTopology(snapshot.grid).distance;
  const distanceToPath = (coord: GridCoord): number => snapshot.pathCenterline.reduce(
    (best, point) => Math.min(best, distance(coord, point)),
    Number.POSITIVE_INFINITY
  );
  const tie = (left: GridCoord, right: GridCoord): number => left.r - right.r || left.q - right.q;
  if (personaId === "turtle_shield") {
...
interface PersonaController {
  ...
  readonly dispatch: (input: GameCommandV6) => boolean;
}

function command(controller: PersonaController, input: GameCommandV6): boolean {
  return controller.dispatch(input);
}

function buildForPersona(
  ...
  for (let pass = 0; pass < PERSONA_QA_LIMITS.buildPassesPerDecision; pass += 1) {
    ...
    if (snapshot.outcome !== "playing") break;
  ...
  if (personaId !== "aggressive_rush") {
    ...
    for (const towerId of towerIdsToUpgrade) {
      for (let attempt = 0; attempt < PERSONA_QA_LIMITS.upgradesPerTowerPerDecision; attempt += 1) {
        if (!command(controller, { schemaVersion: 6, type: "upgradeTower", towerId })) break;
        accepted += 1;
      }
    }
  }
...
function executePersona(
  ...
  tickStep: number,
  journaled = false
): Readonly<{ run: PersonaQaRunV1; finalSnapshot: GameSnapshot; journal?: GameCommandJournal }> {
  ...
  const controller: PersonaController = {
    game,
  ...
  if (command(controller, { schemaVersion: 6, type: "startWave" })) acceptedCommandCount += 1;
  ...
  while (elapsed < simSeconds && game.getSnapshot().outcome === "playing") {
    ...
    if (command(controller, { schemaVersion: 6, type: "tick", units })) acceptedCommandCount += 1;
    elapsed += units;
...
function assertSelectedMapBudgets(content: GameContentRegistry, missionIds: readonly string[]): void {
  for (const missionId of missionIds) {
    const mission = content.missions[missionId];
    if (!mission) throw new Error(`Unknown Persona QA mission "${missionId}".`);
    ...
    if (!map || !Number.isSafeInteger(map.width) || !Number.isSafeInteger(map.height)
      ...
      throw new Error(
        `Persona QA map cell budget exceeded for mission "${missionId}"; maximum is ${PERSONA_QA_LIMITS.mapCells}.`
      );
    }
  }
}
 ...
 */
export function runPersonaQaSuiteV1(content: GameContentRegistry, input: PersonaQaRequestV1): PersonaQaReportV1 {
  const request = normalizeRequest(input);
  for (const missionId of request.missionIds) {
    if (!content.missions[missionId]) throw new Error(`Unknown Persona QA mission "${missionId}".`);
  }
  assertSelectedMapBudgets(content, request.missionIds);
  const canonicalContent = canonicalRegistry(content);
  const runs: PersonaQaRunV1[] = [];
  for (const missionId of request.missionIds) {
    for (const seed of request.seeds) {
      for (const personaId of request.personaIds) {
        runs.push(executePersona(
          canonicalContent,
          missionId,
          seed,
          personaId,
 ...
 */
export function provePersonaQaReplayV1(
  content: GameContentRegistry,
  input: PersonaQaRequestV1
): PersonaQaReplayProofV1 {
  const request = normalizeRequest(input);
  ...
  assertSelectedMapBudgets(content, request.missionIds);
  const canonicalContent = canonicalRegistry(content);
  const executed = executePersona(
    canonicalContent,
    request.missionIds[0]!,
    request.seeds[0]!,
    request.personaIds[0]!,
    request.simSeconds,
    request.tickStep,
    true
  );

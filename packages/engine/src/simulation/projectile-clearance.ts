...
export function projectileAltitudeAtProgress(
  sourceElevation: number,
  targetElevation: number,
  trajectory: BallisticsTrajectoryV1,
...
export function traceProjectileClearanceV1(
  ...
  if (steps <= 1) return Object.freeze({ ok: true, cellInspections: 0 });
  ...
  for (let index = 1; index < line.length - 1; index += 1) {
    ...
    let blockerTag: string | undefined;
    let blockerHeight = Number.NEGATIVE_INFINITY;
    for (const tag of terrain?.tags ?? []) {
      if (!Object.prototype.hasOwnProperty.call(clearance.terrainBlockerHeights, tag)) continue;
    ...
    if (blockerTag === undefined) continue;
    ...
    if (projectileAltitude > blockerElevation + blockerHeight) continue;
    return Object.freeze({
      ok: true,
      cellInspections: inspections,

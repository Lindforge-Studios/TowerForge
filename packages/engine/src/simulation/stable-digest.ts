...
function checkedBudget(name: string, value: number | undefined, fallback: number): number {
...
export function stableDigest(value: unknown, options?: CanonicalStringifyOptions): string {
...
type ContentProjector = (value: unknown) => unknown;
...
function plainObjectDescriptors(value: unknown, context: string): Record<PropertyKey, PropertyDescriptor> {
...
function projectArray(value: unknown, context: string, projectItem: ContentProjector): unknown[] {
...
function projectRecord(value: unknown, context: string, projectValue: ContentProjector): Record<string, unknown> {
...
function projectDefinition(
  ...
  replacements: Readonly<Record<string, ContentProjector>> = {}
): Record<string, unknown> {
  const descriptors = plainObjectDescriptors(value, context);
  const projected: Record<string, unknown> = {};
  for (const key of Object.keys(descriptors)) {
    if (omittedFields.has(key)) continue;
    const descriptor = descriptors[key]!;
    if (!("value" in descriptor) || !descriptor.enumerable) {
...
const projectCurrencyDefinition: ContentProjector = (value) =>
  projectDefinition(value, "Currency definition", OMIT_LABEL_COLOR);
const projectDifficultyDefinition: ContentProjector = (value) =>
  projectDefinition(value, "Difficulty definition", OMIT_LABEL_DESCRIPTION);
const projectTerrainDefinition: ContentProjector = (value) =>
  projectDefinition(value, "Terrain definition", OMIT_LABEL);
const projectAbilityDefinition: ContentProjector = (value) =>
  projectDefinition(value, "Ability definition", OMIT_LABEL);
const projectEnemyDefinition: ContentProjector = (value) =>
  projectDefinition(value, "Enemy definition", OMIT_LABEL_COLOR);
const projectTowerDefinition: ContentProjector = (value) =>
  projectDefinition(value, "Tower definition", OMIT_LABEL);
const projectWaveDefinition: ContentProjector = (value) =>
  projectDefinition(value, "Wave definition", OMIT_LABEL);
const projectObjectiveDefinition: ContentProjector = (value) =>
  projectDefinition(value, "Mission objective definition", OMIT_LABEL);

function projectMetaProgression(value: unknown): Record<string, unknown> {
  return projectDefinition(value, "Meta progression", new Set(), {
    ...
    upgrades: (upgrades) => projectRecord(
      upgrades,
      "Meta upgrades",
      (upgrade) => projectDefinition(upgrade, "Meta upgrade definition", OMIT_LABEL_DESCRIPTION)
    )
  });
}

function projectObjectives(value: unknown): Record<string, unknown> {
...
function projectMissionDefinition(value: unknown): Record<string, unknown> {
...
function projectMissions(value: unknown): Record<string, unknown> {
 ...
 */
export function getSimulationContentDigest(content: GameContentRegistry): string {
  ...
  const simulationContent: Record<string, unknown> = {};
  for (const key of Object.keys(descriptors)) {
    if (EXCLUDED_CONTENT_DOMAINS.has(key)) continue;
    const descriptor = descriptors[key]!;
    if (!("value" in descriptor)) {
      throw new Error(`Simulation content domain "${key}" must be an own data property.`);
    }

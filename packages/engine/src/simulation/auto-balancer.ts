...
export interface AutoBalanceBatchRequestV1 {
  ...
  readonly evaluate: (run: { readonly candidateId: string; readonly seed: string; readonly strategyId: string }) => number;
...
function cloneJson(value: unknown): unknown {
  ...
  if (typeof value === "number") {
    if (!Number.isFinite(value)) throw new Error("Auto-balancer patches require finite JSON numbers.");
    return value;
  }
  if (Array.isArray(value)) {
    if (Object.getPrototypeOf(value) !== Array.prototype) throw new Error("Auto-balancer patches require plain JSON arrays.");
    const descriptors = Object.getOwnPropertyDescriptors(value);
    if (Object.getOwnPropertySymbols(descriptors).length > 0) {
      throw new Error("Auto-balancer patches reject symbol fields.");
    }
    const keys = Object.keys(descriptors).filter((key) => key !== "length");
    if (keys.length !== value.length) throw new Error("Auto-balancer patches reject sparse or extended arrays.");
  ...
  if (typeof value === "object" && value !== null && Object.getPrototypeOf(value) === Object.prototype) {
    ...
    for (const key of Object.keys(descriptors).sort()) {
      ...
      if (!descriptor || !("value" in descriptor) || !descriptor.enumerable) throw new Error("Auto-balancer patches require own JSON data.");
...
function normalizeDimensionStrings(value: readonly string[], label: string, limit: number): readonly string[] {
...
function normalizeCandidates(value: readonly AutoBalanceCandidateV1[]): readonly AutoBalanceCandidateV1[] {
  ...
  const candidates = value.map((candidate) => {
    ...
    if (seen.has(id)) throw new Error(`Duplicate auto-balancer candidate id "${id}".`);
    seen.add(id);
    ...
    return Object.freeze({ id, patch: patch as Readonly<Record<string, unknown>> });
  });
  return Object.freeze(candidates.sort((a, b) => a.id < b.id ? -1 : a.id > b.id ? 1 : 0));
}

export function runAutoBalancerBatch(request: AutoBalanceBatchRequestV1) {
  if (!Number.isFinite(request.baselineScore)) throw new Error("Auto-balancer baselineScore must be finite.");
  ...
  let evaluatedRuns = 0;
  const evaluated: { candidate: AutoBalanceCandidateV1; sum: number; count: number }[] = [];
    ...
    for (const seed of seeds) {
      for (const strategyId of strategyIds) {
        if (request.isCancelled?.()) {
          return Object.freeze({ schemaVersion: 1 as const, status: "cancelled" as const, evaluatedRuns, proposals: Object.freeze([]) });
        }
        const score = request.evaluate({ candidateId: candidate.id, seed, strategyId });
        if (!Number.isFinite(score)) throw new Error(`Auto-balancer candidate "${candidate.id}" produced a non-finite score.`);
        sum += score;
        count += 1;
        evaluatedRuns += 1;
      }
  ...
  const ranked = evaluated.map(({ candidate, sum, count }) => {
    ...
    return {
      id: candidate.id,
      patch: cloneJson(candidate.patch) as Readonly<Record<string, unknown>>,
  ...
  return Object.freeze({ schemaVersion: 1 as const, status: "completed" as const, evaluatedRuns, proposals });
}

...
| Mechanics catalog and modules | catalog v1; `combat` v1/v2/v3; `reactions` and `navigation` v1; `elevation` v1/v2 | Reactions depend on the same mission's active combat v2/v3 profile; elevation v2 adds optional LoS while v1 and v2-without-LoS remain elevation-only; unsupported future versions fail closed and upgrades are explicit |
| Player profile | engine-owned `PlayerProfileV2` codec; v3 planned with rogue runtime | Profile migration never follows project migration implicitly; browser persistence delegates to the codec |
| Engine checkpoint | `GameCheckpointV1`; `towerforge-sim-v1` | Closed codec and engine/content/RNG/state digests gate atomic restore |
...
- Authored elevation: R3.1 adds sparse signed `GridMapDefinition.elevationOverrides` with implicit zero and a closed empty `elevation` v1 mission profile. Elevation is immutable content exposed by `GridMap.elevationAt` and an optional active snapshot section; it is not added to legacy tile rows or checkpoint state. R3.2 extends the module monotonically to v2 with optional deterministic LoS: topology line, elevation, terrain tags, acquisition filtering, and bounded analysis stay in the engine; Studio/MCP request analysis and renderers only copy verdicts. R3.3 extends elevation to v3 with an independent bounded `highGround` sibling. Pairwise tower/enemy range stays inside acquisition, and immediate live-tower damage contributes one existing spatial modifier through the common resolver; no renderer math, mutable state, event, checkpoint field, or new analysis protocol is introduced. See [ADR 0023](docs/adr/0023-opt-in-authored-elevation-foundation.md), [ADR 0024](docs/adr/0024-opt-in-deterministic-elevation-line-of-sight.md), and [ADR 0025](docs/adr/0025-opt-in-authored-high-ground-modifiers.md).
...
- Transactional terraforming: R3.4b introduces a separate opt-in `terraforming` v1 capability rather than extending physics. Its closed engine profile describes authored terrain-tag transitions and an optional elevation policy; mission-scoped validation distinguishes active errors from inactive warnings and never borrows transition IDs from another profile. TowerScript v6 adds the closed `terraformTiles` batch contract and `elevationChanged` vocabulary, while descriptor-safe validation rejects accessors, proxies, sparse arrays, and over-budget inputs without executing them. Accepted runtime C1 stages persistent terrain batches on `authored_routes` and proves every authored route. Accepted C2A builds detached baseline/candidate dynamic resolvers, shares fields by movement profile plus numeric goal, proves all profile×route endpoints and live current/in-progress-next cells, preplans enemy rebinds, and adopts terrain/overrides/resolver/cache/fields/rebinds before compatibility cues and ordered events. Accepted C2B1 adds an internal, solver-free canonical wave/death/phase/TowerScript v1–v6 reachability graph whose terrain fixpoint expands only from applied handlers; it remains unconnected to runtime until C2B2. Failed candidates preserve live gameplay/navigation identities and stats apart from reserved script budgets and one typed diagnostic. C2B2 owns graph integration and exact pre-construction solver budgets; elevation, expiry, checkpoint state, legacy adapters, authoring surfaces, and renderer cues remain later R3.4b slices. See [ADR 0027](docs/adr/0027-opt-in-transactional-terraforming.md).
...
- Difficulty and progression: `content/balance.json` may declare difficulty multipliers plus persistent meta currencies, upgrades, and per-mission rewards. The engine owns `PlayerProfileV2` rules/codecs and explicit launch options; it never reads browser storage. Both generated players use those immutable reducers through the renderer-neutral, app-scoped persistence adapter.
...
Project-v3 map sources may additionally carry a top-level `elevationOverrides` array or its JSON representation in a Tiled property. Values are sparse closed `{q,r,elevation}` safe-integer rows, bounded to 65,536 entries and `-1_000_000..1_000_000`; coordinates are unique and in bounds. Missing cells have elevation `0`, explicit zero rows compile away, and nonzero output is canonical numeric `(r,q)`. In completed R3.1 elevation changes content identity but does not affect terrain, navigation, range, damage, line of sight, physics, or TowerScript. R3.2 may consult immutable elevation only when an enabled, selected elevation v2 profile contains `lineOfSight`; otherwise the legacy acquisition path is unchanged. Snapshot and checkpoint version domains do not change.

...
| Mechanics catalog and modules | catalog v1; `combat` v1/v2/v3; `reactions`, `navigation`, `physics`, and `terraforming` v1; `elevation` v1/v2/v3; `roguelite` v1/v2 | Reactions depend on the same mission's active combat v2/v3 profile; elevation v2 adds optional LoS, v3 adds optional high-ground rules, roguelite v2 adds conditional artifact loot state, and persistent elevation mutation additionally requires the same mission's active terraforming elevation policy; unsupported future versions fail closed and upgrades are explicit |
...
- Rogue-lite synergies and artifact loot: R4.1A adds closed opt-in `roguelite` v1 profiles and optional tower-type `tags`. The pure engine counts live placed instances, derives the greatest reached tier by default or all reached tiers in explicit cumulative mode, and compiles only damage modifiers at stage `run`. R4.2A/B extends the module monotonically to v2 with bounded artifact definitions, typed tower slots, and authored boss loot tables. Drops use a separate domain-separated seeded RNG and a checkpointed battle-local inventory; renderer/player surfaces read only the optional v2 snapshot and `artifactDropped` events. Authored artifact modifiers and `socket:null` are intentionally inert until the separate socket-command slice. Studio and MCP share the engine descriptor and one revision-guarded three-file transaction; both recipes are detached and never enable/select the module. Missing, disabled, unselected, and v1 projects consume no artifact RNG/checkpoint/surface and remain on the earlier path. See [ADR 0030](docs/adr/0030-opt-in-roguelite-tower-synergies.md) and [ADR 0031](docs/adr/0031-opt-in-roguelite-artifact-loot.md).

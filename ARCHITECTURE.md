...
| Mechanics catalog and modules | catalog v1; `combat` v1/v2/v3; `reactions`, `navigation`, `physics`, and `terraforming` v1; `elevation` v1/v2/v3; `roguelite` v1/v2 | Reactions depend on the same mission's active combat v2/v3 profile; elevation v2 adds optional LoS, v3 adds optional high-ground rules, roguelite v2 adds conditional artifact loot/socket state exposed by snapshot v3, and persistent elevation mutation additionally requires the same mission's active terraforming elevation policy; unsupported future versions fail closed and upgrades are explicit |
...
| Command and journal | `GameCommandV1/V2`; `GameCommandJournalV1/V2` | V1 remains byte-compatible; exact artifact socket commands promote new sessions to v2, and both journal versions remain closed validation-only codecs that reference a checkpoint rather than living inside checkpoints/projects |
...
- Rogue-lite synergies, artifact loot, and sockets: R4.1A adds closed opt-in `roguelite` v1 profiles and optional tower-type `tags`. The pure engine counts live placed instances, derives the greatest reached tier by default or all reached tiers in explicit cumulative mode, and compiles damage modifiers at stage `run`. R4.2A/B extends the module monotonically to v2 with bounded artifact definitions, typed tower slots, authored boss loot tables, a domain-separated seeded RNG, and checkpointed battle-local inventory. R4.2C adds exact GameCommand/Journal v2 socket commands, nested artifact checkpoint v2, and snapshot v3. Artifact modifiers affect only immediate damage from the exact socketed live tower; a shared worst-case modifier budget rejects overflow. Studio, MCP and both players consume authoritative snapshot assignments and never duplicate phase or compatibility rules. Missing, disabled, unselected, and v1 projects consume no artifact RNG/checkpoint/surface and remain on the earlier path. See [ADR 0030](docs/adr/0030-opt-in-roguelite-tower-synergies.md), [ADR 0031](docs/adr/0031-opt-in-roguelite-artifact-loot.md), and [ADR 0032](docs/adr/0032-opt-in-roguelite-artifact-socketing.md).
...
- Versioned commands: `packages/engine/src/simulation/commands.ts` owns the closed `GameCommandV1/V2` unions and the only validated dispatcher for deterministic gameplay mutations. V2 contains exact `socketArtifact`/`unsocketArtifact` commands and bounds their identifiers; structurally valid legacy commands retain their v1 representation. The dispatcher canonicalizes untrusted own data descriptors into detached values, bounds JSON signal payloads by depth, node count, and UTF-8 bytes, and rejects transport envelope fields. The legacy headless `SimulationAction` remains an adapter.
...
- Command journal: `packages/engine/src/simulation/journal.ts` owns `GameCommandJournalV1/V2`, `JournaledGameSession`, result normalization, capacity limits, detached export, and validation-only decode. A session preserves journal v1 until the first valid v2-only command, then promotes monotonically; replay accepts both versions. `command-internal.ts` is the single non-public strict parse/execute path shared by the journal and `dispatchGameCommand`. Journal decode validates the embedded checkpoint without creating a map and never executes commands; deterministic execution remains a separate consumer.
